	...
	public void registerSprite(TextureMap map) {
		for (int level = 0; level < LEVELS; level++) {
			...
			for (BlockBakeFrame texture : sellafieldTextures) {
				ResourceLocation spriteLoc = new ResourceLocation("leafia", ROOT_PATH + texture.getPrimaryTexturePath() + "-" + level);
	...
	public void bakeModel(ModelBakeEvent event) {
		for (int level = 0; level < LEVELS; level++) {
			...
			for (int variant = 0; variant < 4; variant++) {
				...
				textureMap.put("all", new ResourceLocation("leafia", ROOT_PATH) + sellafieldTextures[variant].getPrimaryTexturePath() + "-" + level);
